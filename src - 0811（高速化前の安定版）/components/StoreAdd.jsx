...
export default function StoreAdd({ onClose, refreshStores }) {
  ...
  const handleZipcodeBlur = async () => {
    ...
    try {
      const response = await fetch(
        `https://zipcloud.ibsnet.co.jp/api/search?zipcode=${trimmedZip}`
      );
    ...
    <div style={{ padding: 20, maxWidth: 480, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          />
          {streetAddress && (
            <small style={{ color: "#666" }}>※ 番地等を入力してください</small>
          )}
        </div>
